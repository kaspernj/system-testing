# Agent Notes

- Avoid introducing a variable used only once; inline the value unless it improves readability or config reuse.
- Use the `gh` CLI for creating PRs when asked.
- Prefer `async () => await ...` for `awaitery` `timeout` callbacks when no functional difference.
- Run `npm run typecheck` after changing JS files.
- Run relevant checks for files that were changed or created.
- When bumping package versions, update both `package.json` and `spec/dummy/package.json`.
- Keep JSDoc `@typedef` blocks immediately after imports.
- Keep single-tag JSDoc blocks on one line when they fit (for example `/** @returns {string | undefined} */`).
- Keep `if`/`else if` conditions on one line when they fit within 160 characters.
- Avoid putting condition logic inside assignment expressions; prefer explicit `if/else` branches.
- When typed option objects are required by contract, pass through explicit provided values and let unsupported values fail loudly at the callee; do not silently narrow or drop invalid values just to keep going.
- In system tests, when the next step is `interact(...)`, prefer selector-based `interact("[data-testid='...']", ...)` calls over finding a throwaway element handle first so retry/error handling stays active.
- In system tests, if a spec needs to clear and/or enter an input value, use the shared `Browser`/`SystemTest` helpers `clear(...)`, `fill(...)`, or `clearAndFill(...)` (fast native defaults; slow keyboard / `js` strategies are explicit opt-in overrides) instead of open-coding a click-clear-sendKeys sequence. `clearAndSendKeys(...)` is a thin alias of `clearAndFill` kept for existing call sites.
- In system tests, if a spec needs to scroll an offscreen element into view, add or use a shared package helper on `Browser`/`SystemTest` (for example `scrollIntoView(...)` / `scrollTestIdIntoView(...)`) instead of project-local DOM query wrappers.
- In app-flow system tests, use `visit` as the single initial app entry point by default, then navigate through visible app UI, links, and actions. A second `visit`, refresh, or direct route jump in one example needs a concrete documented reason.
- For per-example browser cleanup such as auth reset, prefer lifecycle support in `SystemTest.run(...)` / `useSystemTest*` (for example `onTeardown`) over putting destructive cleanup into app bootstrap callbacks like `onInitialize`.
- If a downstream app exposes a bug rooted in `system-testing`, fix it in `system-testing` first and open the PR here before asking the app repo to carry a dependency override or downstream workaround.
- Do not run Android emulator/Appium specs locally when `/dev/kvm` is unavailable; they are too slow without hardware virtualization. Use CI/Tensorbuzz for those checks instead, and run non-emulator validation locally.
- Detect a native app session from the resolved driver capabilities (Appium `type` with empty/absent `browserName`, via `isAppiumNativeAppDriverConfig`), not only `SYSTEM_TEST_HOST === "native"`. Appium native runs use `SYSTEM_TEST_HOST=dist`, so a host-only check wrongly treats them as web: it starts the dist HTTP server, drives a `driverVisit`, starts the client WebSocket after the app launches, and uses the short web connect timeout — causing "timeout while waiting for client WebSocket connection". Native app sessions must start the client WebSocket before the driver and use the native-safe connect timeout.
- The Android CI setup must install every SDK platform required by the dummy app build. Expo SDK 54 currently builds with compileSdk 36, so keep `platforms;android-36` in `scripts/setup-android-emulator.js` along with the emulator system image packages.
